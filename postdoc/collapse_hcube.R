...
if(test_simplex_func){
  ...
  n_pts <- 100
  shape_simplex <- 1.2
  simplex_weights <- blending_simplex_path(n_simplex, n_pts, shape_simplex, 2)
...
collapse_objective_givens <- function(
    ...
    w,                   # matrix of blending weights, n_steps rows
    angular_vel_fun,     # your function "angular_velocity(R)"
  ...
  sub_ps <- rbind(theta_init[p_inds], 
                  matrix(p, n_p, length(p_inds), byrow = T))
  ...
  dev_loss <- sum((t(t(sub_ps[-1,]) - theta_init[p_inds]))^2) * 10 * loss_weights[2]
  
  total_loss <- alignment_loss + av_loss + dev_loss
...
p_init <- rep(theta_init[p_inds], n_p)
...
all_ps <- matrix(theta_init, n_steps, length_full_p, byrow = T)
sub_ps <- rbind(theta_init[p_inds], 
                matrix(best_par, n_p, length(p_inds), byrow = T))
sub_ps_weighted <- w %*% sub_ps
all_ps[,p_inds] <- sub_ps_weighted

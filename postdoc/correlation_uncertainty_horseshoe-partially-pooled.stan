...
transformed data{
  ...
  real<lower = 0> scale_global = p0r ./ (pr - p0r) / sqrt(pr); // scale for the half -t prior for tau
...
parameters{
  ...
  // group deviations
  real global_logratio;
  vector[p] local_logratio;
  real<lower=0> sd_local_logratio;
...
transformed parameters{
  ...
  vector[2] global_ratios;
  global_ratios[1] = exp(global_logratio);
  global_ratios[2] = 1 / global_ratios[1];
  
  array[2] vector[p] local_ratios;
  local_ratios[1] = exp(local_logratio * sd_local_logratio);
  local_ratios[2] = 1 / local_ratios[1];
  
  //combining everything together
  lambda =  aux1_local .* sqrt(aux2_local);
  lambda_tilde =  sqrt(c^2 * lambda^2 ./ (c^2 + tau^2 * lambda^2));
  for(i in 1:2){
    x[,i] =  to_array_1d(to_vector(z[,i]) .* 
      lambda_tilde .* 
      local_ratios[i] * 
      global_ratios[i] * 
      tau);
...
model{
  ...
  // proportional deviations model for local and global effects
  local_logratio ~ std_normal();
  global_logratio ~ std_normal();
  sd_local_logratio ~ std_normal();

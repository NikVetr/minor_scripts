...
data {
  int<lower=3> p;  // only p is provided as data
}
...
parameters {
  ...
  //-------------------------------------------------------------------
  real<lower=-1,upper=1> signed_y_array[p-1];

  //-------------------------------------------------------------------
  // 3) The "sphere directions"
  //    We can't do ragged arrays. We'll just store p-1 of them,
  //    each dimension p-2. For iteration i, we only use the first (p-i-1) entries.
  //    Each is a unit_vector, so dot_self(...)=1, but we won't always use all components.
  //-------------------------------------------------------------------
  unit_vector[p-2] theta_array[p-1];
}

transformed parameters {
  //-------------------------------------------------------------------
  // 1) Transform raw_dirichlet into a decreasing vector L that sums to p
  //    E.g. sort in descending order, scale by p
  //-------------------------------------------------------------------
  vector[p] L = sort_desc(raw_dirichlet) * p;
  ...
  for (i in 1:(p-1)) {
    // absolute value => [0,1], assuming the truncated normal ensures |signed_y| <=1
    y_array[i] = fabs(signed_y_array[i]);
    // sign flip in {-1, +1}, if signed_y=0 we can pick +1 arbitrarily:
    sign_flip_array[i] = (signed_y_array[i] < 0) ? -1 : 1;
  }

  //-------------------------------------------------------------------
  // 3) Flatten all the partial "theta" draws into one big vector "theta_values"
  //    We'll have sum_{i=1..(p-1)}(p-i-1) entries total = (p-1)(p-2)/2
  //-------------------------------------------------------------------
  // Pre-allocate:
...

  {
    int pos = 1;
    for (i in 1:(p-1)) {
      // how many coords do we actually need from row i?
      int used = p - i - 1;
      if (used > 0) {
        // just take the first 'used' components of theta_array[i]
        // (since each row is dimension p-2)
...
model {
  // 1) raw_dirichlet ~ Dirichlet(alpha=?)
  //    For example, alpha=1 => uniform simplex
  //    Or alpha=some hyperparameter if you want
  raw_dirichlet ~ dirichlet(rep_vector(1.0, p));

  // 2) signed_y_array ~ normal(0, 0.2) T[-1,1]
  //    The <lower=-1, upper=1> enforces truncated domain
  for (i in 1:(p-1)) {
    // no extra line needed; the domain is truncated by param constraints
    // you could add a prior if you want: e.g. target += normal_lpdf(signed_y_array[i] | 0, 0.2);
    signed_y_array[i] ~ normal(0, 0.2);
  }

  // 3) theta_array[i] automatically a unit_vector => uniform on sphere
  //    If you want to add a prior, it’s by default uniform on the sphere
  //    so no extra statement needed
}

generated quantities {
  // We'll just call construct_corrmat to produce C_samp

...
#specify word params and sample strings
par(xpd = NA)
nwords <- 10
ncw <- sample(3:8, replace = T, size = nwords)
words <- sapply(ncw, function(i) paste0(sample(c(LETTERS, letters, 0:9), i), collapse = ""))
wcols <- sample(colors(distinct = T), nwords)
rgb2col <- function(x){ rgb(x[1,], x[2,], x[3,])}
wcols <- rgb2col(col2rgb(wcols)/255/1.5)
...
#start plotting
plot.new()
plot.window(xlim = range(dw$x0) + c(-diff(range(dw$x0)), diff(range(dw$x0))) * 3, 
            ylim = range(dw$y0) + c(0, diff(range(dw$y0))) * 6)
points(dw$x0, dw$y0, pch = 19, col = wcols)
...
wsh <- max(dw$h) * 1
dw$y1 <- max(dw$y0) + diff(par("usr")[3:4])/5 + (dw$row - 1) * (max(dw$h) + wsh)
dw$x1 <- median(dw$x0)
dw$rb <- dw$lb <- 0
...
# dw <- cbind(dw, do.call(cbind, row_targets))
...
horiz_line_max <- sapply(1:nro, function(ri) 
  mean(dw$y1[dw$row == ri] - dw$h[dw$row == ri])) 
...
#test function
source("~/scripts/minor_scripts/postdoc/geometric_word_labels_function.R")
...
#start plotting
plot.new()
plot.window(xlim = range(x0) + c(-diff(range(x0)), diff(range(x0))) * 3, 
            ylim = range(y0) + c(0, diff(range(y0))) * 6)
points(x0, y0, pch = 19, col = wcols)
stacked_word_labels(words, wcols, x0, y0, nro = 2, offx = 0.2, variable_priority = T, offset_y = 0, offset_x = -5)
#### end ####
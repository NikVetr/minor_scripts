...
k <- 20 #number of those variables that are known (and of interest)
...
fits_3 <- lapply(1:m, function(i){
  fit <- lm(y[,i] ~ x[,1:k] + matrix(rnorm(n*n_PCs_to_use), n, n_PCs_to_use))
  return(list(
    coefs = summary(fit)$coefficients[2:(k+1),1],
    pvals = summary(fit)$coefficients[2:(k+1),4],
    resids = fit$resid

...
blood_signs <- setNames(sign(human_blood_res$beta[human_blood_res$adj_p < alpha_fdr & 
                                                    (human_blood_res$I2 > i2_filter)]), blood_genes)
muscle_signs <- setNames(sign(human_muscle_res$beta[human_muscle_res$adj_p < alpha_fdr & 
                                                      (human_muscle_res$I2 > i2_filter)]), muscle_genes)
...
for(tissue in tissues[1]){
  ...
  yval <- motrpac_data$logFC
  yval_se <- motrpac_data$logFC_se
  
  xval <- xdat$beta[match(motrpac_data$human_gene_symbol, xdat$symbol)]
  xval_se <- xdat$beta_se[match(motrpac_data$human_gene_symbol, xdat$symbol)]
  
  valid_entries <- !is.na(xval) & !is.na(yval) & xval_se > 0 & yval_se > 0
  yval_se <- yval_se[valid_entries]
  yval <- yval[valid_entries]
  
  xval_se <- xval_se[valid_entries]
  xval <- xval[valid_entries]
  
  #do analysis
  
  #inverse-variance weighted correlation first?
  x_err <- cbind(xval, yval)
  sd_x_err <- cbind(xval_se, yval_se)

...
get_rect_info <- function(rectangles) {
  ...
  properties <- list(
    ...
    total_width = rectangles$w2 + rectangles$w3 + prop_dist*rectangles$h2,
    total_height = rectangles$h1 + rectangles$h2 * (prop_dist + 1),
...
)
{
  ...
  obj_fun <- function(x) {
    ...
    # Dimensions that follow from x
    h1 <- ar1 * w1            # b1
    w3 <- h2 / ar3            # b3
    # b2 has width w2, height h2
    
    # 1) Base objective = total area 
    area_b1 <- w1 * h1               # or ar1 * w1^2
    area_b2 <- w2 * h2
    area_b3 <- w3 * h2               # = (h2^2)/ar3
    total_area <- area_b1 + area_b2 + area_b3
    ...
    curr_height <- h1 + prop_dist*h2 + h2
    ...
    b2_iar_value <- ifelse(b2_iar < mean_iar2f, 
                           ...
                           min(mod_b2_iar, mean_iar2f - mod_b2_iar))
    b2_iar_penalty <- -dnorm(b2_iar_value, mean=0, sd=sd_iar2f, log=T) * 
      ifelse(b2_iar < mean_iar2f, big_penalty * 5, iar2f_weight) #imposes strong constraint
  ...
  if(two_stage_iar2f){
    ...
    iar2f_weight <- 0
    res <- do.call(rbind, lapply(1:nrep, function(nri){
      if(nri > 1){

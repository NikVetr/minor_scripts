data {
  ...
  matrix[p, p-1] Q;                   // zero‑sum orthonormal basis
}

transformed data {
    real sd_scale_factor = inv_sqrt(1 - 1.0 / p);
}
...
transformed parameters {
  vector[p] z = sd_scale_factor * (Q * z_raw);
  real<lower = 0> sigma =  exp(logsigma); // noise std
